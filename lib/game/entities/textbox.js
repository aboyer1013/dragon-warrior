	...
	.defines(function () {
		...
		ig.global.TextBox = ig.global.Entity.extend({
			...
			init: function (x, y, settings) {
				...
				this.selector = ig.game.spawnEntity(ig.global.Selector, this.pos.x + this.tileSize, this.pos.y + (this.tileSize * 2), {
					hidden: true
				});
				this.setMenuItems();
			},
			...
			update: function () {
				this.parent();
				this.elapsedTicks++;
				if (this.elapsedTicks % 1 === 0 && this.currentRow <= this.totalRows) {
					this.currentRow += 2;
				}
			...
			/**
			 * These set the coords between other menu Items.
			 * It will guide the selector arrow to know how to move between them.
			 */
			// TODO I don't think I need this anymore - just go by drawData's row and column.
			// setMenuItemCoords: function () {
			// 	_.map(this.contents, _.bind(function (content, i) {
			// 		if (this.isMenuItem(content)) {
			// 			if (_.isUndefined(content.column)) {
			// 				content.column = 0;
			// 			}
			// 			content.menuItemCoords = {x: content.column, y: i};
			// 		}
			// 		return content;
			// 	}, this));
			// },
			setDefaultSelectedMenuItem: function () {
				var menuItems = _.sortBy(this.getMenuItems(), ['col', 'row']);
				var defaultSelected = _.find(menuItems, {defaultSelected:true});
				var topLeftMenuItem;
			...
			isMenuItem: function (obj) {
				return _.isFunction(obj.onSelect);
			}
		});
	});
