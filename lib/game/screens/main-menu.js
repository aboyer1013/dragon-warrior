ig.module(
	...
	.defines(function () {
		...
		ig.global.MainMenuScreen = ig.global.Game.extend({
				...
				this.menuSettings = {
					...
					questsMenu: {
						...
						contents: (_.bind(function () {
							return this.setQuestMenuItems(true);
						}, this)())
					},
					msgSpeedMenu: {
						name: 'msgSpeedMenu',
			...
			setQuestMenuItems: function (showEmptyInstead) {
				var result = [];
				var heroName = '';
				...
				_.forEach(Data.adventureLogs, function (log, i) {
					...
					if (!_.isEmpty(log) && !showEmptyInstead) {
						heroName = _.padEnd(log.heroName.substring(0, 4), 4);
					...
					if (ig.input.pressed('DOWN')) {
						...
						if (this.isMenuOpenAndEnabled(selectedMenu)) {
							...
							switch (selectedMenu.name) {
								case 'mainMenu':
									...
									break;
								case 'userInputMenu':
									if (selectedMenuItem.text === 'END' || selectedMenuItem.text === 'L') {
									...
									break;
								case 'msgSpeedMenu':
									// FIXME (where I left off ^_^) This breaks when changing text speed from existing adv log.
									this.saveCurrentGame();
									this.closeAllBut(this.getMenuByName('mainMenu'));
									break;
								case 'questsMenu':
									if (this.getMenuByName('mainMenu').getSelectedMenuItem().text === 'COPY A QUEST') {
								...
								case 'existingQuestsMenu':
									if (this.getMenuByName('mainMenu').getSelectedMenuItem().text === 'CHANGE MESSAGE SPEED') {
										this.setActiveAndOpen(this.getMenuByName('msgSpeedMenu'));
									} else if (this.getMenuByName('mainMenu').getSelectedMenuItem().text === 'COPY A QUEST') {
										this.setActiveAndOpen(this.getMenuByName('questsMenu'));
									}
								default:
									break;
						...
						switch (this.state.selectedMenu.name) {
							...
							case 'existingQuestsMenu':
								...
								this.setActiveMenu(this.getMenuByName('mainMenu'));
								break;
							case 'userInputMenu':
			...
			onConfirmMenuYes: function () {
				var getAdventureLogIdx = function (text) {
					return text.match((/adventure log (\d)/i))[1] - 1;
				};
				var fromIdx = getAdventureLogIdx(this.getMenuByName('existingQuestsMenu').getSelectedMenuItem().text);
				var toIdx = getAdventureLogIdx(this.getMenuByName('questsMenu').getSelectedMenuItem().text);

				this.copyAdventureLogTo(fromIdx, toIdx);
				this.closeAllBut(this.getMenuByName('mainMenu'));
			},
			...
			copyAdventureLogTo: function (fromIdx, toIdx) {
				Data.saveAdventureLog(toIdx, Data.adventureLogs[fromIdx]);
			},
			...
			saveCurrentGame: function () {
				var adventureLogIdx = _.parseInt(_.last(this.getMenuByName('questsMenu').getSelectedMenuItem().text.split(''))) - 1;
				var data = {
					heroName: this.getMenuByName('userInputMenu').userInput,
					msgSpeed: this.getMenuByName('msgSpeedMenu').getSelectedMenuItem().text.toLowerCase()
				};

				Data.saveAdventureLog(adventureLogIdx, data);
			},
			...
			updateName: function (selectedMenuItem) {
				...
				userInputMenu.userInput = _.trim(input.join(''), '*');
