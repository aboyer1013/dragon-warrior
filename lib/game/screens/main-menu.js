	...
	.defines(function () {
		...
		ig.global.MainMenuScreen = ig.global.Game.extend({
			...
			getEraseQuestContent: function (data) {
				...
				return [
					{text: data.heroName, type: 'static'},
					{text: 'LEVEL   ' + level, type: 'static'},
			...
			getQuestMenuItems: function (showEmptyInstead) {
				...
				_.forEach(Data.adventureLogs, function (log, i) {
					...
					if (!_.isEmpty(log) && !showEmptyInstead) {
						heroName = log.heroName && _.padEnd(log.heroName.substring(0, 4), 4);
							...
							switch (selectedMenu.name) {
								...
								case 'msgSpeedMenu':
									...
									this.saveAdventureLog(advLogIdx, {
										...
										heroName: this.getHeroData(activeQuestMenu, advLogIdx).heroName,
										heroLevel: this.getHeroData(activeQuestMenu, advLogIdx).heroLevel
									});
			...
			getHeroData: function (activeQuestMenu, advLogIdx) {
				if (activeQuestMenu.name === 'existingQuestsMenu') {
					return {
						heroName: Data.adventureLogs[advLogIdx].heroName,
						heroLevel: Data.adventureLogs[advLogIdx].heroLevel
					};
				}
				return {
