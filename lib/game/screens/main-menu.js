		...
		ig.global.MainMenuScreen = ig.global.Game.extend({
			...
			getMainMenuItems: function () {
				...
				if (!Data.areAdventureLogsFull()) {
					...
					items.push({text: 'CONTINUE A QUEST', type: 'menuItem'});
				}
				if (Data.hasAdventureLogs()) {
					items.push({text: 'CHANGE MESSAGE SPEED', type: 'menuItem'})
					if (!Data.areAdventureLogsFull()) {
						...
						if (this.isMenuOpenAndEnabled(selectedMenu)) {
							...
							switch (selectedMenu.name) {
								...
								case 'userInputMenu':
									...
									break;
								case 'msgSpeedMenu':
									this.saveAdventureLog(this.getAdventureLogIdx(activeQuestMenu.getSelectedMenuItem().text), {
			...
			updateQuestsMenuItems: function () {
			...
			onConfirmMenuYes: function () {
				var fromIdx = this.getAdventureLogIdx(this.getMenuByName('existingQuestsMenu').getSelectedMenuItem().text);
				var toIdx = this.getAdventureLogIdx(this.getMenuByName('questsMenu').getSelectedMenuItem().text);
