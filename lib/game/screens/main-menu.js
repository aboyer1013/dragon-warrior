	...
	.defines(function () {
		...
		ig.global.MainMenuScreen = ig.global.Game.extend({
				...
				this.menuSettings = {
					...
					questMenu: {
						...
						contents: (function () {
							...
							_.forEach(Data.getOccupiedAdventureLogs(), function (isOccupied, i) {
								if (!isOccupied) {
									result.push({text: 'ADVENTURE LOG ' + (i + 1), type: 'menuItem'})
								}
							});
							return result;
						}())
						...
						contents: (function () {
							...
							_.forEach(Data.adventureLogs, function (log, i) {
								if (!_.isEmpty(log)) {
									...
									result.push({text: 'ADVENTURE LOG ' + (i + 1) + ':' + heroName, type: 'menuItem'})
								}
							});
							return result;
						}())
					...
					if (ig.input.pressed('DOWN')) {
						...
						if (this.isMenuOpenAndEnabled(selectedMenu)) {
							...
							switch (selectedMenu.name) {
								case 'mainMenu':
									if (selectedMenuItem.text === 'CONTINUE A QUEST') {
			...
			saveCurrentGame: function () {
				var adventureLogIdx = _.parseInt(_.last(this.getMenuByName('questMenu').getSelectedMenuItem().text.split(''))) - 1;
				var data = {
				...
				Data.saveCurrentGame(adventureLogIdx, data);
			},
			...
			onExistingQuestsMenuTransitionUpdated: function (change) {
				this.onQuestMenuTransitionUpdated(change);
			},
			...
			onMainMenuBeginANewQuest: function () {
				this.setActiveAndOpen(this.getMenuByName('questMenu'));
			}
		});
	});
